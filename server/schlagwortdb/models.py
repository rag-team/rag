...
class Schlagwort(Base):
    ...
    geschaeftsfeld: Mapped[str] = mapped_column(
        type_=String(255), server_default="empty"
    )
    ...
    felder: Mapped[List["Feld"]] = relationship(
        "Feld", back_populates="schlagwort_obj"
    )
...
class Feld(Base):
    ...
    feldname: Mapped[str] = mapped_column(
        type_=String(255), server_default="empty", unique=True
    )
    schlagwort: Mapped[int] = mapped_column(ForeignKey("Schlagworte.pkey"))
